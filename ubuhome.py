...
with col1:
    ...
    def about():
        with col3:
            st.title("Hi, I know why you're here")
            st.title("Because Your want To Use Best Chat Bot AI ")
            st.title("In The Worldddd")
    ...
    def addchat():
        ...
        with col_input:
            user_input = st.text_input("พิมพ์ข้อความของคุณที่นี่...", key="chat_input") # ช่องกรอกข้อความ
            if st.button("ส่งข้อความ"):  # เมื่อกดปุ่มส่ง
                if user_input.strip(): # ถ้าผู้ใช้กรอกข้อความ
    ...
    if st.page_link in ["เพิ่มแชทและคุยกับแชทบอท"]:  # เงื่อนไขเพื่อแสดงกล่องข้อความเฉพาะหน้าคุยกับบอทและเพิ่มแชท
        with col3:
            st.write("🤖 Ubu Chat Bot AI ")
        ...
        col_empty, col_input = st.columns([0.1, 0.9])  # ใช้ 2 คอลัมน์
        with col_input:
            user_input = st.text_input("พิมพ์ข้อความของคุณที่นี่...", key="chat_input")
    ...
    pages = [
        st.Page(about, title="เกี่ยวกับเรา", icon="ℹ️"),
